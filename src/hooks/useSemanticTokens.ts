...
// ============================================================================
// SEMANTIC TOKENS HOOK
// Injects semantic CSS variables from compiled semantic token bundle
// ============================================================================
...
interface UseSemanticTokensResult extends SemanticTokensState {
...
const STYLE_ELEMENT_ID = 'ds-os-semantic-tokens';

/**
 * Hook to manage semantic theme tokens with multi-mode support
 * 
 * Usage:
 * ```tsx
 * const { isLoading, tokenCount, compile, getTokenValue, setMode, currentMode } = useSemanticTokens({
 *   projectId,
 *   tenantId,
 *   userId,
 ...
 * });
 * 
 * // Get a specific semantic token's CSS variable
...
export function useSemanticTokens({
  ...
  autoInject?: boolean;
  defaultMode?: string;
}): UseSemanticTokensResult {
  const [state, setState] = useState<SemanticTokensState>({
    isLoading: true,
    isCompiling: false,
    error: null,
    tokenCount: 0,
  ...
  const applyMode = useCallback((mode: string) => {
    ...
    console.log(`[SEMANTIC-TOKENS] Applied theme mode: ${mode}`);
  }, []);
  ...
  const injectCSS = useCallback(async (cssUrl?: string, cssContent?: string) => {
    ...
    if (cssUrl) {
      try {
        ...
        console.log('[SEMANTIC-TOKENS] Injected CSS from CDN:', cssUrl);
        return true;
      } catch (error) {
        console.warn('[SEMANTIC-TOKENS] Failed to load CSS from CDN, falling back to inline:', error);
    ...
    if (cssContent) {
      ...
      console.log('[SEMANTIC-TOKENS] Injected semantic token CSS inline (fallback)');
      return true;
    }
  ...
  const loadBundle = useCallback(async () => {
    ...
    try {
      ...
      const bundle = await convex.action('tokenCompiler:getSemanticBundle' as any, { 
        projectId, 
        tenantId, 
        userId 
      });
      ...
      if (bundle.jsonUrl) {
        try {
          ...
          if (response.ok) {
            ...
            console.log('[SEMANTIC-TOKENS] Loaded JSON map from CDN:', bundle.jsonUrl);
          } else {
            throw new Error('Failed to fetch from CDN');
          }
        } catch (error) {
          console.warn('[SEMANTIC-TOKENS] Failed to load JSON from CDN, using fallback:', error);
      ...
      console.log(`[SEMANTIC-TOKENS] Loaded semantic bundle: ${bundle.tokenCount} tokens, version ${bundle.version}, modes: ${modes.join(', ')}`);
    } catch (err) {
      if (!mountedRef.current) return;
      console.error('[SEMANTIC-TOKENS] Failed to load semantic bundle:', err);
      setState(prev => ({
        ...prev,
        isLoading: false,
        error: err instanceof Error ? err.message : 'Failed to load semantic tokens',
      }));
    }
  ...
  const compile = useCallback(async () => {
    ...
    try {
      ...
      console.log('[SEMANTIC-TOKENS] Starting semantic token compilation...');
      const result = await convex.action('tokenCompiler:compileSemanticTokens' as any, { 
        projectId, 
        tenantId, 
        userId 
      });
      console.log(`[SEMANTIC-TOKENS] Compilation complete: ${result.tokenCount} tokens, ${result.cssSize} bytes CSS`);
      ...
      console.error('[SEMANTIC-TOKENS] Compilation failed:', err);
      if (mountedRef.current) {
        setState(prev => ({
          ...prev,
          isCompiling: false,
  ...
  const setMode = useCallback((mode: string) => {
    if (!state.availableModes.includes(mode) && mode !== 'default') {
      console.warn(`[SEMANTIC-TOKENS] Mode "${mode}" not available. Available modes: ${state.availableModes.join(', ')}`);
      return;
    }
    
    setState(prev => ({ ...prev, currentMode: mode }));
    applyMode(mode);
    console.log(`[SEMANTIC-TOKENS] Switched to mode: ${mode}`);
