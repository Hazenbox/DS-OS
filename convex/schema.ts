...
export default defineSchema({
  ...
  // Token Bundles - Compiled CSS/JSON bundles for efficient loading
  // Global bundles contain all project tokens as CSS variables
  // Component bundles contain only tokens used by that component
  tokenBundles: defineTable({
    tenantId: v.id("tenants"),
    projectId: v.id("projects"),
    type: v.union(v.literal("global"), v.literal("component")),
    componentId: v.optional(v.id("components")), // Only for component bundles
    version: v.string(), // Semantic version for cache busting
    cssContent: v.optional(v.string()), // CSS :root { --token: value } for global bundles
    jsonContent: v.string(), // JSON map { tokenName: cssVarName }
    tokenCount: v.number(), // Number of tokens in bundle
    modes: v.optional(v.array(v.string())), // Available modes (light, dark, etc.)
    createdAt: v.number(),
    updatedAt: v.number(),
  })
    .index("by_tenant", ["tenantId"])
